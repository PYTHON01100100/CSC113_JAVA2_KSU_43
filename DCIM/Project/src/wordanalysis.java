...
occurrence list, containing w’s line number and word position.
	...
	public void readFileAndAnalyse(String fn) throws IOException {  this.filename = fn; n=NoOfWords(fn); int  k=maxLength(filename);
			...
			while((line = br.readLine()) != null) {
				...
				for(int i=0;i<s.length;i++) {
					...
					if(arrayOfDifferentLengths[words[x].length()] !=null && arrayOfDifferentLengths[words[x].length()].findWord(words[x])) {
						...
						nk=arrayOfDifferentLengths[words[x].length()].getNoOfData();
						nk++;
						arrayOfDifferentLengths[words[x].length()].setNoOfData(nk);
...
occurring in the text file. Case 1= O(1), Case 2 = O(1)
	 */
	private int totalWordsForLength1(int len) throws Exception{
		int no=0;
	...
	public int totalWordsForLength(int len) throws Exception{
		int no=0;
		no=arrayOfDifferentLengths[len].getNoOfData();
		if(no>0)
		return no;
		else
			return 0;
	 ...
	 */
		public LinkedList<WordOccurrence> ccurrences(String s) throws Exception{
			...
			if(arrayOfDifferentLengths[s.length()] !=null && arrayOfDifferentLengths[s.length()].findWord(s)) {
				...
				System.out.println("there is an error try again"); return null;}
			return null;
		}
		/*
...
operation). O(n)
	...
	public boolean checkAdjacent(String s1,String s2) throws Exception {
		...
		//if both not null we must make the cur in the head
		...
		while(t1.retrieve()!=null) {
			...
			t1.findfirst();
			LinkedList<WordOccurrence> pre=t1;
			t1.findNext();
			if(t1!=null) {
			while(!t1.last()) {
				if(t1.retrieve().lineNo==pre.retrieve().lineNo) {
					if((pre.retrieve().position+1)==(t1.retrieve().position))
						return true;
			}
				pre=t1;
				t1.findNext();
		...
		private int maxLength(String fn)throws FileNotFoundException,IOException {
			...
			input=new Scanner(f); 
			int max=0; input.useDelimiter("[^A-Z-a-z\\u0027]+");
